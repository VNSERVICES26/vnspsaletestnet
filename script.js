...
async function updateBalances() {
  ...
  const vns = new web3.eth.Contract(VNS_ABI, VNS_TOKEN_ADDRESS);
  const balance = await usdt.methods.balanceOf(userAccount).call();
  document.getElementById("usdtBalance").innerText = `USDT Balance: ${web3.utils.fromWei(balance)} USDT`;

  const sellerBalance = await vns.methods.balanceOf(SELLER_WALLET).call();
  document.getElementById("sellerBalance").innerText = `Seller VNS Balance: ${web3.utils.fromWei(sellerBalance)} VNS`;
}

async function approveUSDT() {
  const usdtAmount = document.getElementById("usdtAmount").value;
  if (!usdtAmount) return alert("Enter USDT amount!");

  const usdt = new web3.eth.Contract(USDT_ABI, USDT_ADDRESS);
  const amountInWei = web3.utils.toWei(usdtAmount);

  try {
    await usdt.methods.approve(PRESALE_ADDRESS, amountInWei).send({ from: userAccount });
...
async function buyVNS() {
  ...
  const presale = new web3.eth.Contract(PRESALE_ABI, PRESALE_ADDRESS);
  const amountInWei = web3.utils.toWei(usdtAmount);

  try {
    await presale.methods.buyVNS(amountInWei).send({ from: userAccount });
    document.getElementById("status").innerText = "🚀 VNS Purchased!";
    updateBalances();
  } catch (err) {
...
function copyAddress() {
  ...
  document.getElementById("status").innerText = "📋 Address Copied!";
}

document.getElementById("usdtAmount").addEventListener("input", function () {
  const usdt = parseFloat(this.value || "0");
  const vnsRate = 1; // 1 USDT = 1 VNS (Change this if needed)
  document.getElementById("vnsToReceive").innerText = `You will get: ${usdt * vnsRate} VNS`;
});
